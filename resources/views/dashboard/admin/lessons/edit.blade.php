...
@section('content')
    @include('layouts.dashboard.partials.breadcrumb', [
        'component' => trans('messages.edit_lesson'),
    ])
    <div class="container-fluid px-4">
        <!-- Header Section -->
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="h3 text-gray-800 mb-0">{{ trans('messages.edit_lesson') }}</h2>
        ...
        <div class="row">
            <div class="col-12 mb-4">
                <div class="card border-0 shadow components-section">
                    <div class="card-body">
                        <form action="{{ route('lessons.update', ['lesson' => $lesson->id]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row mb-4">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('name_ar') is-invalid @enderror" id="name_ar" name="name_ar"
                                            value="{{ old('name_ar', $lesson->name_ar) }}" placeholder="{{ trans('messages.enter_name_ar') }}">
                                        @error('name_ar')
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('name_en') is-invalid @enderror" id="name_en" name="name_en"
                                            value="{{ old('name_en', $lesson->name_en) }}" placeholder="{{ trans('messages.enter_name_en') }}">
                                        @error('name_en')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('description_ar') is-invalid @enderror" id="description_ar" name="description_ar" rows="2"
                                            placeholder="{{ trans('messages.enter_description_ar') }}">{{ old('description_ar', $lesson->description_ar) }}</textarea>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('description_en') is-invalid @enderror" id="description_en" name="description_en" rows="2"
                                            placeholder="{{ trans('messages.enter_description_en') }}">{{ old('description_en', $lesson->description_en) }}</textarea>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('unit_id') is-invalid @enderror" id="unit_id" name="unit_id">
                                            ...
                                            @foreach ($units as $unit)
                                                <option value="{{ $unit->id }}" {{ $unit->id == $lesson->unit_id ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? $unit->name_ar : $unit->name_en }}</option>
                                            @endforeach
                                        </select>
                                        @error('unit_id')
                                            <span class="invalid-feedback">{{ $message }}</span>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <button type="submit"
                                        class="btn btn-primary">{{ trans('messages.update_lesson') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
