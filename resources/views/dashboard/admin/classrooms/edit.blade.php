...
@section('content')
    @include('layouts.dashboard.partials.breadcrumb', [
        'component' => trans('messages.edit_classroom'),
    ])
    <div class="container-fluid px-4">
        <!-- Header Section -->
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="h3 text-gray-800 mb-0">{{ trans('messages.edit_classroom') }}</h2>
                    ...
                    <div class="card-body">
                        <form action="{{ route('classrooms.update', $classroom->id) }}" method="POST">
                            ...
                            <div class="row mb-4">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('name') is-invalid @enderror mb-2" id="name" name="name"
                                            value="{{ old('name', $classroom->name) }}" placeholder="{{ trans('messages.enter_name') }}">
                                        @error('name')
                                        ...
                                        <select class="form-control @error('school_id') is-invalid @enderror mb-2" id="school_id" name="school_id">
                                            ...
                                            @foreach ($schools as $school)
                                                <option value="{{ $school->id }}" {{ old('school_id', $classroom->school_id) == $school->id ? 'selected' : '' }}>
                                                    {{ app()->getLocale() == 'ar' ? $school->name_ar : $school->name_en }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('school_id')
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        <label for="capacity" class="form-label">{{ trans('messages.capacity') }}</label>
                                        <input type="number"
                                            class="form-control @error('capacity') is-invalid @enderror mb-2"
                                            id="capacity" name="capacity"
                                            value="{{ old('capacity', $classroom->capacity) }}" min="0" 
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('description') is-invalid @enderror mb-2"
                                            ...
                                            placeholder="{{ trans('messages.description') }}">{{ old('description', $classroom->description) }}</textarea>
                                        @error('description')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <button type="submit" class="btn btn-primary ml-3">{{ trans('messages.update_classroom') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
