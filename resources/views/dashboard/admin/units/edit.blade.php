...
@section('content')
    @include('layouts.dashboard.partials.breadcrumb', [
        'component' => trans('messages.edit_unit'),
    ])
    <div class="container-fluid px-4">
        <!-- Header Section -->
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="h3 text-gray-800 mb-0">{{ trans('messages.edit_unit') }}</h2>
                    ...
                    <div class="card-body">
                        <form action="{{ route('units.update', ['unit' => $unit->id]) }}" method="POST" >
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('name_ar') is-invalid @enderror" id="name_ar" name="name_ar"
                                            value="{{ old('name_ar', $unit->name_ar) }}" placeholder="{{ trans('messages.enter_name_ar') }}">
                                        @error('name_ar')
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('name_en') is-invalid @enderror" id="name_en" name="name_en"
                                            value="{{ old('name_en', $unit->name_en) }}" placeholder="{{ trans('messages.enter_name_en') }}">
                                        @error('name_en')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('description_ar') is-invalid @enderror" id="description_ar" name="description_ar" rows="2"
                                            placeholder="{{ trans('messages.enter_description_ar') }}">{{ old('description_ar', $unit->description_ar) }}</textarea>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('description_en') is-invalid @enderror" id="description_en" name="description_en" rows="2"
                                            placeholder="{{ trans('messages.enter_description_en') }}">{{ old('description_en', $unit->description_en) }}</textarea>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('grade_id') is-invalid @enderror" id="grade_id" name="grade_id">
                                            ...
                                            @foreach ($grades as $grade)
                                                <option value="{{ $grade->id }}" {{ $grade->id == $unit->grade_id ? 'selected' : '' }}>{{ $grade->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('grade_id')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('subject_id') is-invalid @enderror" id="subject_id" name="subject_id">
                                            ...
                                            @foreach ($subjects as $subject)
                                                <option value="{{ $subject->id }}" {{ $subject->id == $unit->subject_id ? 'selected' : '' }}>{{ $subject->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('subject_id')
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <button type="submit"
                                        class="btn btn-primary">{{ trans('messages.update_unit') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
