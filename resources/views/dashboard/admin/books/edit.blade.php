...
@section('content')
    @include('layouts.dashboard.partials.breadcrumb', [
        'component' => trans('messages.edit_book'),
    ])
    <div class="container-fluid px-4">
        <!-- Header Section -->
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="h3 text-gray-800 mb-0">{{ trans('messages.edit_book') }}</h2>
                ...
                <div class="card border-0 shadow components-section">
                    <div class="card-body">
                        <form action="{{ route('books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row mb-4">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('title') is-invalid @enderror mb-2" id="title" name="title"
                                            value="{{ old('title', $book->title) }}" placeholder="{{ trans('messages.enter_title') }}">
                                        @error('title')
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('author') is-invalid @enderror mb-2" id="author" name="author"
                                            value="{{ old('author', $book->author) }}" placeholder="{{ trans('messages.enter_author') }}">
                                        @error('author')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="row">
                                        <div class="col-6">
                                            <div class="form-group">
                                                <label for="publication_year" class="form-label">{{ trans('messages.publication_year') }}</label>
                                                <input type="number" class="form-control @error('publication_year') is-invalid @enderror mb-2" id="publication_year" name="publication_year"
                                        ...
                                        <div class="col-6">
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control @error('description') is-invalid @enderror mb-2" id="description" name="description" rows="2"
                                                    placeholder="{{ trans('messages.description') }}">{{ old('description', $book->description) }}</textarea>
                                                @error('description')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                            ...
                            <div class="row mb-4">
                                        ...
                                        <select class="form-control @error('subject_id') is-invalid @enderror mb-2" id="subject_id" name="subject_id">
                                            ...
                                            @foreach ($subjects as $subject)
                                                <option value="{{ $subject->id }}" {{ old('subject_id', $book->subject_id) == $subject->id ? 'selected' : '' }}>
                                                    {{ app()->getLocale() == 'ar' ? $subject->name : $subject->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('subject_id')
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        @if ($book->cover_image)
                                            <img src="{{ asset($book->cover_image) }}" alt="Cover Image" class="img-thumbnail mt-2" width="100">
                                        @endif
                                        @error('cover_image')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <button type="submit" class="btn btn-primary ml-3">{{ trans('messages.update_book') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
